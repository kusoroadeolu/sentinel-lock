...
import io.github.kusoroadeolu.sentinellock.entities.CompletableSaveResult;
...
public class FencingTokenValidator {
    ...
    public void validateAndSave(@NonNull CompletableSaveResult future, @NonNull Lease lease, @NonNull Object value){
        final var redisOps = this.synchronizerTemplate.opsForValue().getOperations();
        var res = redisOps.execute(new FencingTokenSessionCallback(lease, value , clientTemplate ,sentinelLockConfigProperties, clientConfigProperties));
        future.complete(res);
    }
        ...
        public @NonNull SaveResult execute(@NonNull RedisOperations ops) throws DataAccessException {
            ...
            try {
                ...
                if (isNull(synchronizer) || synchronizer.currentFencingToken() != fencingToken){
                    ops.unwatch();
                    return new SaveResult.Failed(SaveResult.Failed.Cause.ACQUIRED);
                }
                ...
                if (res.isEmpty()) {
                    ...
                    return new SaveResult.Failed(SaveResult.Failed.Cause.RACE_CONDITION);
                }
                ...
                return new SaveResult.Failed(SaveResult.Failed.Cause.ERR);
            }
        }
    }
}
